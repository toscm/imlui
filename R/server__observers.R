server__setup_observers <- function(data) {
    ...
    observe({
        ...
        # Not quite sure why this is necessary. Maybe to trigger this on any input change, however if this is the
        # case, converting to all reactive values to a list seems a bit costly in terms of runtime. Maybe improve
        # later.
        session$doBookmark()
        # Update the query string (triggers onBookmark and onBookmarked callback functions). Based on:
