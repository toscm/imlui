...
# datasets <- dplyr::filter(config$Datasets, Symbol != "")$Name
...
# ON SESSION START
# (no explicit function required, because server func is only run once on session start):
# Check if parameter code is present in URL. If yes, we're coming back from the the redirection to Github as part of
# the Oauth2 flow and need to set credentials$user_auth as well as the login cookie.
# Maybe better: store cookie "OAuth2_Flow_Started=TRUE; OAuth2_Target=https://github.com/auth" or similar before
# starting OAuth2 flow. Then, not only check for param `code` in URL but also for cookie. If both are present, check
# if authentication worked and delete cookie.
...
# Helper .Func
asSVG <- function(.pname, .func, .arglist, .fname=deparse(substitute(.func))) {
	width <- PAW()
	height <- PAH() - 240
...
# .argfunc = (closure) function to return arglist required for `.func` (usually a reactive)
renderSVG <- function(.name, .func, .argfunc) {
